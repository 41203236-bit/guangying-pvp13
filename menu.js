...
const ROOMS_ROOT = 'rooms_v2';
function roomRef(code){ return ref(db, `${ROOMS_ROOT}/${code}`); }
...
async function cancelDisconnectHandler(){
  ...
  try {
    ...
    await onDisconnect(ref(db, `${ROOMS_ROOT}/${currentRoomCode}/players/${currentSlot}`)).cancel();
  } catch (e) {
...
async function setupDisconnectHandler(){
  ...
  try {
    ...
    else await onDisconnect(ref(db, `${ROOMS_ROOT}/${currentRoomCode}/players/${slot}`)).set({ joined:false, name:'', faction:'', role:'', ready:false, clientId:'' });
  } catch (e) {
...
async function joinRoom(){
  ...
  const hostSlot = data.hostSlot || data.host || 'O';
  const hostFaction = data.players?.[hostSlot]?.faction || '';
  if (hostFaction && hostFaction === selectedFaction) {
    setStatus(`陣營重複，請改成 ${hostFaction === 'light' ? '暗' : '光'} 後再加入。`);
    return;
  }
...
async function startBattle(){
  if (els.startBtn.disabled || !currentRoomCode || !roomData) return;

  const playerO = roomData.players?.O;
  const playerX = roomData.players?.X;

  if (!playerO?.joined || !playerX?.joined) {
    setStatus('雙方尚未都加入房間。');
    return;
  }

  if (!playerO?.role || !playerO?.faction || !playerX?.role || !playerX?.faction) {
    setStatus('雙方角色資料不完整。');
    return;
  }

  if (playerO.faction === playerX.faction) {
    setStatus('雙方陣營不能相同。');
    return;
  }
